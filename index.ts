...
export const isGlobalIssueOrPRList = (url: URL | Location = location): boolean => ['issues', 'pulls'].includes(url.pathname.split('/', 2)[1]);
collect.set('isGlobalIssueOrPRList', [
...
export const isIssueOrPRList = (url: URL | Location = location): boolean => isGlobalIssueOrPRList(url) || isRepoIssueOrPRList(url);
collect.set('isIssueOrPRList', combinedTestOnly);
...
export const isOrganizationIssueOrPR = (url: URL | Location = location): boolean => /^orgs\/[^/]+\/teams\/[^/]+($|\/IssueOrPRs)/.test(getCleanPathname(url));
collect.set('isOrganizationIssueOrPR', [
	'https://github.com/orgs/refined-github/teams/core-team/IssueOrPRs?pinned=1',
	'https://github.com/orgs/refined-github/teams/core-team/IssueOrPRs/1',
...
export const isRepoTaxonomyIssueOrPRList = (url: URL | Location = location): boolean => /^labels\/.+|^milestones\/\d+(?!\/edit)/.test(getRepoPath(url)!);
collect.set('isRepoTaxonomyIssueOrPRList', [
...
export const isRepoIssueOrPRList = (url: URL | Location = location): boolean =>
	isRepoPRList(url) ||
	isRepoIssueList(url) ||
	isRepoTaxonomyIssueOrPRList(url);
collect.set('isRepoIssueOrPRList', combinedTestOnly);
